...
"""
This is print functions module.
It included each of command print function. 
"""

def print_welcome_message():
    # Welcoming message from help command
    print('\n\tWelcome to JLOCK!\n'
              '\tby J. Matta\n'
              '\t(c) November 2022\n'
              '\n\tType \'jlock.py -h\' or \'jlock.py -help\' for a list of available commands.\n')

def print_separation_line(line_char, line_size):
    # print separate line with characters. It print character line length with line size.
    separation_str = line_char*line_size
    print(f'\n\t{separation_str}\n')
    return separation_str

def print_help_welcome():
    #print the very first welcoming message
    print('\t\t', 'JLOCK Help')
    print('\t\t', '(c)2022 J. Matta (edited by Chiho) ')
...
def extract_locked_file_content(file_name):
    # open the locked (encoded) message files
